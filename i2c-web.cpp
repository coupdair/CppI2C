...
std::cout<<std::endl<<"dur="<<i<<std::endl<<std::endl;
  ...
  {std::cout<<__func__<<" page"<<std::endl;
    ...
    if(id%2)//odd
    {
      c.infoMC2SAamplitudeOrVCIM.setGUI(true);
      c.infoMC2SAtestNdiscri.setGUI(true);
    }//odd
    ///GUI template
    ini(c);
    ///GUI static
    ...
    ///GUI action
        if(request().request_method()=="POST")
        {
            ...
            c.infoMC2SAoption.load(context());
            ...
            {///MC2SA resistor
              ...
              if(verbose>1) std::cout<<"resistor//R5="<<resistor.to_string()<<std::endl;
